...
output "secretsmanager_secret_arn_ec2_private_key" {
  ...
  value       = var.connect_via_session_manager ? null : aws_secretsmanager_secret.liberty_app_instance_host_private_key[0].arn
}

output "ec2_private_key" {
  value     = var.connect_via_session_manager ? null : tls_private_key.liberty_app_instance_host_key[0].private_key_openssh
  sensitive = true
}
...
output "asg_alb_dns_name" {
  ...
  value       = "http://${aws_lb.liberty_app_alb.dns_name}/liberty-app"
}

output "asg_name" {
  description = "Auto Scaling Group name"
  value       = aws_autoscaling_group.liberty_app_asg.name
}
