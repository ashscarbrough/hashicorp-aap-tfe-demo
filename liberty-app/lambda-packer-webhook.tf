...
resource "aws_iam_role" "packer_webhook_lambda" {
  name = "hcp-packer-liberty-app-webhook-lambda-role"
...
resource "aws_lambda_function" "packer_webhook" {
  function_name    = "hcp-packer-liberty-app-webhook"
  ...
  environment {
    variables = {
      ...
      PACKER_BUCKET_FILTER   = "liberty-app"  # Hardcoded to the Liberty App bucket, as this Lambda is specific to that application
    }
  }
}
