...
resource "aws_security_group" "alb_sg" {
  ...
  description = "Allow inbound HTTP/HTTPS to the liberty-app ALB"
  vpc_id      = var.vpc_id
...
resource "aws_security_group" "liberty_app_instance_sg" {
...
resource "aws_security_group_rule" "instance_from_alb_http" {
  ...
  to_port                  = 9080
  protocol                 = "tcp"
  security_group_id        = aws_security_group.liberty_app_instance_sg.id
  source_security_group_id = aws_security_group.alb_sg.id
}

### Egress rule for EC2 instances - allow all outbound traffic
resource "aws_vpc_security_group_egress_rule" "liberty_app_instance" {
  security_group_id = aws_security_group.liberty_app_instance_sg.id
