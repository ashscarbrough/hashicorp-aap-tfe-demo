...
resource "aws_lb_target_group_attachment" "liberty_base" {
  ...
  target_id        = aws_instance.aap_tfe_demo_host.id
  port             = 9080
...
resource "null_resource" "wait_for_alb_healthy" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      echo "Waiting for instance ${aws_instance.aap_tfe_demo_host.id} to become healthy..."
      ...
      until [ $ATTEMPTS -ge $MAX_ATTEMPTS ]; do
        STATE=$(aws elbv2 describe-target-health \
          ...
          --targets Id=${aws_instance.aap_tfe_demo_host.id} \
